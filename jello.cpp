...
struct point mouseForceOrigin, mouseForceDest, mouseForceStrength;
int selected = 0;
...
int shear=0, bend=0, structural=1, pause=0, viewingMode=0, saveScreenToFile=0, coordinateAxes=0, showFPS=0;

struct world jello;

int windowWidth, windowHeight;

int init_time = time(NULL), final_time, frame_count = 0;
...
void display()
{
  ...
  GLfloat lKd6[] = { 1.0, 0.44, 0.38, 1.0 };
  GLfloat lKs6[] = { 1.0, 0.44, 0.38, 1.0 };
  ...
  // show the inclined plane if presented
  if (jello.incPlanePresent == 1)
    showInclinedPlane(&jello);
  
  // show the mouse-control force
  if (selected == 1)
      drawArrow(mouseForceOrigin, mouseForceDest, 0.1, 0.1, 0.792, 0.733, 0.914);  // purple arrow
 
  glutSwapBuffers();

  // show the FPS and resolution
  if (showFPS == 1)
  {
      frame_count++;
      ...
      {
          printf("\rResolution: %dx%d, FPS: %.2f", windowHeight, windowWidth, (double)frame_count / (final_time - init_time));
          frame_count = 0;
